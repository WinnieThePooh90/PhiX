...
**Notenauswertung für Lehrkräfte** — Klausuren, Tests, mündliche Noten, Projekte, GFS und Gesamtübersicht in einer Anwendung. Entwickelt für den Schulalltag (u. a. Gymnasium in Baden-Württemberg), als Web-App, Docker-Server oder Windows-Desktop nutzbar.

| | |
|---|---|
| **Aktueller Build** | `224` (siehe [`docs/APP_VERSION.md`](docs/APP_VERSION.md)) |
...
- **Tests** — analog zu Klausuren, optional pro Kurs
...
- **GFS** — Großes Fachgespräch mit Thema, Art, Halbjahr und Note
...
- Mehrere **Kurse** pro Benutzer, Favoriten, Kurs-Einstellungen (Gewichtung, Tests/Projekte/GFS)
- **Schülerverwaltung** pro Kurs und zentral (Schuljahre, Klassenstufen)
- **Analyse** — Notenverteilung, Einzelübersichten
- **Export** — Excel/PDF (u. a. Übersicht, Klausurtabellen)
- **Klassenlehrer** — Geldsammlungen, Anwesenheit, Sammellisten, Bemerkungen (optional pro Kurs)
...
- Backup/Import, Benutzerverwaltung (Admin)
...
In `package.json` der drei Hauptpakete steht sie als SemVer **`"<BUILD>.0.0"`** (z. B. `224.0.0`), weil npm und electron-builder eine SemVer-Zeichenkette erwarten — die **Major-Komponente** entspricht dem Build.
...
Hauptabhängigkeiten (Auszug): React (MIT), Express (MIT), Prisma (Apache-2.0), SheetJS/xlsx (Apache-2.0), jsPDF (MIT), Electron (MIT, Desktop-Builds), Lucide Icons (ISC).
